<script lang="ts">
  ...
  import { activeTool, annotations, loadAnnotations, addAnnotation } from '../stores/annotations.store';
  ...
  const toolIcons: Record<Annotation['type'], string> = {
    highlight: '🖍️',
    note: '📝',
    ink: '✏️',
    underline: 'U̲',
...
<div class="toolbar">
  ...
  <!-- Page navigation -->
  <button class="btn icon" on:click={() => currentPage.update(p => Math.max(1, p - 1))} title="Pagina precedente">◀</button>
  <span class="page-info">
    <input type="number" bind:value={$currentPage} min="1" max={$totalPages} />
    <span class="total">/ {$totalPages}</span>
  </span>
  <button class="btn icon" on:click={() => currentPage.update(p => Math.min($totalPages, p + 1))} title="Pagina successiva">▶</button>

  <div class="separator"></div>

  <!-- Zoom -->
  <button class="btn icon" on:click={() => zoomBy(-0.25)} title="Rimpicciolisci">−</button>
  ...
  {#each ['highlight', 'note', 'ink', 'underline', 'strikeout'] as tool}
    <button
      ...
      title={tool}
    >
      {toolIcons[tool as Annotation['type']]}
    </button>
  {/each}
  ...
  <!-- Save menu -->
  <button class="btn" on:click={() => { showSaveMenu = !showSaveMenu; loadAndCacheAnnotations(); }}>
    💾
  </button>
  {#if showSaveMenu}
    <div class="save-menu">
      <button class="btn" on:click={downloadXFDFFile}>Scarica annotazioni (XFDF)</button>
      <span class="hint">PDF annotato e XFDF saranno disponibili dopo l'integrazione di pdf-lib</span>
    </div>
  {/if}
</div>
